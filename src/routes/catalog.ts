...
catalog.post("/", authMiddleware, async (c) => {
  ...
  await c.env.D1.prepare("INSERT INTO catalog_items (id, name, description, price, qty, image_id, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)")
    .bind(itemId, name, description || null, price, qty, imageId, userId)
    .run();
...
catalog.post("/cart", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (item.user_id === userId) {
      return c.json({ error: "Anda tidak dapat membeli produk Anda sendiri" }, 400);
    }
    ...
    if (existingCartItem) {
      ...
      await c.env.D1.prepare("UPDATE cart SET quantity = ?, updated_at = current_timestamp WHERE user_id = ? AND item_id = ?").bind(newQuantity, userId, itemId).run();
    } else {
      const cartId = crypto.randomUUID();
      await c.env.D1.prepare("INSERT INTO cart (id, user_id, item_id, quantity) VALUES (?, ?, ?, ?)").bind(cartId, userId, itemId, quantity).run();
    }
...
catalog.put("/cart/:itemId", authMiddleware, async (c) => {
  ...
  try {
    ...
    const result = await c.env.D1.prepare("UPDATE cart SET quantity = ?, updated_at = current_timestamp WHERE user_id = ? AND item_id = ?").bind(quantity, userId, itemId).run();
...
catalog.delete("/cart/:itemId", authMiddleware, async (c) => {
  ...
  try {
    const result = await c.env.D1.prepare("DELETE FROM cart WHERE user_id = ? AND item_id = ?").bind(userId, itemId).run();
...
catalog.put("/:id", authMiddleware, async (c) => {
  ...
  await c.env.D1.prepare("UPDATE catalog_items SET name = ?, description = ?, price = ?, qty = ?, image_id = ?, updated_at = current_timestamp WHERE id = ?")
    .bind(name ?? existingItem.name, description ?? existingItem.description, price ?? existingItem.price, qty ?? existingItem.qty, imageId, id)
    .run();
...
catalog.delete("/:id", authMiddleware, async (c) => {
  ...
  const query = role === "seller" ? "SELECT * FROM catalog_items WHERE id = ? AND user_id = ?" : "SELECT * FROM catalog_items WHERE id = ?";
  const params = role === "seller" ? [id, userId] : [id];

  const existingItem = (await c.env.D1.prepare(query)
    .bind(...params)
    .first()) as CatalogItem | undefined;
...
catalog.post("/checkout", authMiddleware, async (c) => {
  ...
  for (const cartItem of cartItems.results as unknown as (CartItemWithStock & { is_archived: number })[]) {
    ...
    if (cartItem.seller_id === buyerId) {
      return c.json({ error: `Anda tidak dapat membeli produk Anda sendiri: ${cartItem.name}` }, 400);
    }
  ...
  for (const { item, existingItem } of stockChecks) {
    ...
    try {
      ...
      await c.env.D1.prepare("INSERT INTO transactions (id, buyer_id, seller_id, item_id, quantity, amount, status) VALUES (?, ?, ?, ?, ?, ?, 'completed')")
        .bind(transactionId, buyerId, sellerId, item.item_id, item.quantity, amount)
        .run();
    } catch {
    ...
    await c.env.D1.prepare("UPDATE catalog_items SET qty = ?, updated_at = current_timestamp WHERE id = ?").bind(newQty, item.item_id).run();
  }
